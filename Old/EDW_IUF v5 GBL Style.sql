...
where ((a11.SHIP_CANCEL_DATE > DATE '2012-12-30'  ) or (a11.PLANNED_STOCKED_DATE > DATE '2012-12-30'))
...
--DROP TABLE  SRAA_SAND.STAGE_STYLE_CATEGORY;
 ...
 CREATE MULTISET  TABLE SRAA_SAND.STAGE_STYLE_CATEGORY AS  (Select NUMBER_SEQ, OPR_BRD_STY_ID, CATEGORY from  SRAA_SAND.EDW_IUF  
	Qualify
	row_number() Over (partition by OPR_BRD_STY_ID order by NUMBER_SEQ desc)= 1 ) WITH DATA;
...
SET CATEGORY = (SELECT STAGE_STYLE_CATEGORY.CATEGORY FROM STAGE_STYLE_CATEGORY 
	...
	WHERE SRAA_SAND.EDW_IUF.CATEGORY IS NULL ;
...
SET MetricShipDate = CASE
		...
		END
	Where  (SRAA_SAND.EDW_IUF.SALES_TERMS_CODE = 'DDP' OR SRAA_SAND.EDW_IUF.SALES_TERMS_CODE = 'DDU')  AND SRAA_SAND.EDW_IUF.TRADE_LANE_TYPE = 'Domestic' ;
...
SET MetricShipDate = CASE
		...
		WHEN SRAA_SAND.EDW_IUF.ACTUAL_STOCKED_LCL_DATE IS NOT NULL THEN 'SD'
		END
	Where (SRAA_SAND.EDW_IUF.SALES_TERMS_CODE = 'DDP' OR SRAA_SAND.EDW_IUF.SALES_TERMS_CODE = 'DDU')  AND SRAA_SAND.EDW_IUF.TRADE_LANE_TYPE = 'International' ;
...
UPDATE SRAA_SAND.EDW_IUF
SET Lateness= CASE
		WHEN  (SRAA_SAND.EDW_IUF.SHIP_MODE_CD = 'A' AND SRAA_SAND.EDW_IUF.SALES_TERMS_CODE = 'CFR') AND (SRAA_SAND.EDW_IUF.MetricShipDate - SRAA_SAND.EDW_IUF.Ship_Cancel_Date >2) THEN 'Late'
		WHEN not (SRAA_SAND.EDW_IUF.SHIP_MODE_CD = 'A' AND SRAA_SAND.EDW_IUF.SALES_TERMS_CODE = 'CFR') AND (SRAA_SAND.EDW_IUF.MetricShipDate - SRAA_SAND.EDW_IUF.CTRT_SHP_DT >2) THEN 'Late'
		Else ' OnTime' END
...
UPDATE SRAA_SAND.EDW_IUF
SET DAYS_LATE= CASE
		WHEN  (SRAA_SAND.EDW_IUF.SHIP_MODE_CD = 'A' AND SRAA_SAND.EDW_IUF.SALES_TERMS_CODE = 'CFR') Then SRAA_SAND.EDW_IUF.MetricShipDate - SRAA_SAND.EDW_IUF.Ship_Cancel_Date 
		WHEN not (SRAA_SAND.EDW_IUF.SHIP_MODE_CD = 'A' AND SRAA_SAND.EDW_IUF.SALES_TERMS_CODE = 'CFR') THEN SRAA_SAND.EDW_IUF.MetricShipDate - SRAA_SAND.EDW_IUF.CTRT_SHP_DT
		END
	where SRAA_SAND.EDW_IUF.MetricShipDate is not null;