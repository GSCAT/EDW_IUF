--DROP TABLE SRAA_SAND.EDW_IUF_TRLR;

CREATE MULTISET TABLE SRAA_SAND.EDW_IUF_TRLR ,NO FALLBACK ,
     NO BEFORE JOURNAL,
     NO AFTER JOURNAL,
     CHECKSUM = DEFAULT,
     ...
     (
      ...
      BRD_STY_ITM_DESC VARCHAR(250) CHARACTER SET LATIN NOT CASESPECIFIC,
     ...
     BRD_STY_CLR_KEY DECIMAL(18,0),
      ...
      BUYING_AGENT_DESC VARCHAR(400) CHARACTER SET LATIN NOT CASESPECIFIC,
      ...
      PAR_VENDOR_LEGAL_DESC VARCHAR(400) CHARACTER SET LATIN NOT CASESPECIFIC,
      VENDOR_LEGAL_DESC VARCHAR(400) CHARACTER SET LATIN NOT CASESPECIFIC,
      SHP_RSN_TYP_DESC VARCHAR(250) CHARACTER SET LATIN NOT CASESPECIFIC,
...
insert into SRAA_SAND.EDW_IUF_TRLR
select
...
case
		...
		when a112.AGT_DEPT_ABBR_DESC is null then null
		else 'Category Other'
		end as Category,
...
DROP TABLE  SRAA_SAND.STAGE_STYLE_CATEGORY;
 ...
 CREATE MULTISET  TABLE SRAA_SAND.STAGE_STYLE_CATEGORY AS  (Select OPR_BRD_STY_ID, CATEGORY from  SRAA_SAND.EDW_IUF_TRLR  
	Qualify
	row_number() Over (partition by OPR_BRD_STY_ID order by OPR_BRD_STY_ID desc)= 1 ) WITH DATA;

--update any DPO that does not have a category if there is a style match above
UPDATE SRAA_SAND.EDW_IUF_TRLR
SET CATEGORY = (SELECT STAGE_STYLE_CATEGORY.CATEGORY FROM STAGE_STYLE_CATEGORY 
	WHERE SRAA_SAND.EDW_IUF_TRLR.OPR_BRD_STY_ID = STAGE_STYLE_CATEGORY.OPR_BRD_STY_ID)
	WHERE SRAA_SAND.EDW_IUF_TRLR.CATEGORY IS NULL ;
	
-- Start of MetricShipDate logic
-- 1) Set FOB/CFR Ocean to LP
UPDATE SRAA_SAND.EDW_IUF_TRLR
SET MetricShipDate = SRAA_SAND.EDW_IUF_TRLR.ACTUAL_LP_LCL_DATE, ShipDateChoice = 'LP'
	WHERE  (SRAA_SAND.EDW_IUF_TRLR.SALES_TERMS_CODE = 'FOB' OR SRAA_SAND.EDW_IUF_TRLR.SALES_TERMS_CODE = 'CFR')
	AND SRAA_SAND.EDW_IUF_TRLR.SHIP_MODE_CD = 'O'
	AND SRAA_SAND.EDW_IUF_TRLR.ACTUAL_LP_LCL_DATE IS NOT NULL;

-- 2) Set FOB Air to OC
UPDATE SRAA_SAND.EDW_IUF_TRLR
SET MetricShipDate = SRAA_SAND.EDW_IUF_TRLR.ACTUAL_ORIGIN_CONSOL_LCL_DATE, ShipDateChoice = 'OC'
	WHERE  (SRAA_SAND.EDW_IUF_TRLR.SALES_TERMS_CODE = 'FOB')
	AND SRAA_SAND.EDW_IUF_TRLR.SHIP_MODE_CD = 'A'
	AND SRAA_SAND.EDW_IUF_TRLR.ACTUAL_ORIGIN_CONSOL_LCL_DATE IS NOT NULL;

-- Define Trade Lane Type
Update SRAA_SAND.EDW_IUF_TRLR
Set Trade_Lane_Type = Case 
			when  SRAA_SAND.EDW_IUF_TRLR.XFR_PT_COUNTRY_CODE=SRAA_SAND.EDW_IUF_TRLR.DEST_COUNTRY_CODE  then 'Domestic'
			ELSE 'International' End; 


--3) Update MetricShipDate to Dom Depart
UPDATE SRAA_SAND.EDW_IUF_TRLR
SET MetricShipDate = SRAA_SAND.EDW_IUF_TRLR.ACTUAL_DOM_DEPART_LCL_DATE, ShipDateChoice = 'DD'
	WHERE  (SRAA_SAND.EDW_IUF_TRLR.SALES_TERMS_CODE = 'FOB')
	AND SRAA_SAND.EDW_IUF_TRLR.SHIP_MODE_CD = 'M'
	AND SRAA_SAND.EDW_IUF_TRLR.TRADE_LANE_TYPE = 'Domestic'
	AND SRAA_SAND.EDW_IUF_TRLR.ACTUAL_DOM_DEPART_LCL_DATE IS NOT NULL;

--4) Update MetricShipDate to Origin Consol
UPDATE SRAA_SAND.EDW_IUF_TRLR
SET MetricShipDate = SRAA_SAND.EDW_IUF_TRLR.ACTUAL_ORIGIN_CONSOL_LCL_DATE, ShipDateChoice = 'OC'
	WHERE  (SRAA_SAND.EDW_IUF_TRLR.SALES_TERMS_CODE = 'FCA')
	AND SRAA_SAND.EDW_IUF_TRLR.ACTUAL_ORIGIN_CONSOL_LCL_DATE IS NOT NULL;

-- 5) Update MetricShipDate to Dom Depart
UPDATE SRAA_SAND.EDW_IUF_TRLR
SET MetricShipDate = CASE
		WHEN  SRAA_SAND.EDW_IUF_TRLR.ACTUAL_DOM_DEPART_LCL_DATE IS NOT NULL THEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_DOM_DEPART_LCL_DATE
		WHEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_IN_DC_LCL_DATE IS NOT NULL THEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_IN_DC_LCL_DATE
		WHEN ACTUAL_STOCKED_LCL_DATE IS NOT NULL THEN ACTUAL_STOCKED_LCL_DATE
		END,
		
		ShipDateChoice = CASE
		WHEN  SRAA_SAND.EDW_IUF_TRLR.ACTUAL_DOM_DEPART_LCL_DATE IS NOT NULL THEN 'DD'
		WHEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_IN_DC_LCL_DATE IS NOT NULL THEN 'InDC'
		WHEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_STOCKED_LCL_DATE IS NOT NULL THEN 'SD'
		END
	Where  (SRAA_SAND.EDW_IUF_TRLR.SALES_TERMS_CODE = 'DDP' OR SRAA_SAND.EDW_IUF_TRLR.SALES_TERMS_CODE = 'DDU')  AND SRAA_SAND.EDW_IUF_TRLR.TRADE_LANE_TYPE = 'Domestic' ;

-- 6) Update MetricShipDate to Load Port
UPDATE SRAA_SAND.EDW_IUF_TRLR
SET MetricShipDate = CASE
		WHEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_LP_LCL_DATE IS NOT NULL THEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_LP_LCL_DATE
		WHEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_IN_DC_LCL_DATE IS NOT NULL THEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_IN_DC_LCL_DATE
		WHEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_STOCKED_LCL_DATE IS NOT NULL THEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_STOCKED_LCL_DATE
		END,
		
				ShipDateChoice = CASE
		WHEN  SRAA_SAND.EDW_IUF_TRLR.ACTUAL_LP_LCL_DATE IS NOT NULL THEN 'LP'
		WHEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_IN_DC_LCL_DATE IS NOT NULL THEN 'InDC'
		WHEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_STOCKED_LCL_DATE IS NOT NULL THEN 'SD'
		END
	Where (SRAA_SAND.EDW_IUF_TRLR.SALES_TERMS_CODE = 'DDP' OR SRAA_SAND.EDW_IUF_TRLR.SALES_TERMS_CODE = 'DDU')  AND SRAA_SAND.EDW_IUF_TRLR.TRADE_LANE_TYPE = 'International' ;

-- 7) Set CFR Motor and Air
UPDATE SRAA_SAND.EDW_IUF_TRLR
SET MetricShipDate = CASE
		WHEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_ORIGIN_CONSOL_LCL_DATE IS NOT NULL THEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_ORIGIN_CONSOL_LCL_DATE
		WHEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_IN_DC_LCL_DATE IS NOT NULL THEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_IN_DC_LCL_DATE
		WHEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_STOCKED_LCL_DATE IS NOT NULL THEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_STOCKED_LCL_DATE
		END,
		ShipDateChoice = CASE
		WHEN  SRAA_SAND.EDW_IUF_TRLR.ACTUAL_ORIGIN_CONSOL_LCL_DATE IS NOT NULL THEN 'OC'
		WHEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_IN_DC_LCL_DATE IS NOT NULL THEN 'InDC'
		WHEN SRAA_SAND.EDW_IUF_TRLR.ACTUAL_STOCKED_LCL_DATE IS NOT NULL THEN 'SD'
		END
	Where  (SRAA_SAND.EDW_IUF_TRLR.SALES_TERMS_CODE = 'CFR') AND (SRAA_SAND.EDW_IUF_TRLR.SHIP_MODE_CD = 'M' OR SRAA_SAND.EDW_IUF_TRLR.SHIP_MODE_CD ='A') ;

-- 8) Set all EXW to XF
UPDATE SRAA_SAND.EDW_IUF_TRLR
SET MetricShipDate = SRAA_SAND.EDW_IUF_TRLR.ACTUAL_X_FACTORY_LCL_DATE, ShipDateChoice = 'XF'
	WHERE  (SRAA_SAND.EDW_IUF_TRLR.SALES_TERMS_CODE = 'EXW')
	AND SRAA_SAND.EDW_IUF_TRLR.ACTUAL_X_FACTORY_LCL_DATE IS NOT NULL;

-- 9) TNR exception
UPDATE SRAA_SAND.EDW_IUF_TRLR
SET MetricShipDate = SRAA_SAND.EDW_IUF_TRLR.ACTUAL_X_FACTORY_LCL_DATE, ShipDateChoice = 'XF'
	WHERE  (SRAA_SAND.EDW_IUF_TRLR.Program_Type = 'TNR')
	AND SRAA_SAND.EDW_IUF_TRLR.ACTUAL_X_FACTORY_LCL_DATE IS NOT NULL;
...
UPDATE SRAA_SAND.EDW_IUF_TRLR
SET Lateness= CASE
		WHEN  (SRAA_SAND.EDW_IUF_TRLR.SHIP_MODE_CD = 'A' AND SRAA_SAND.EDW_IUF_TRLR.SALES_TERMS_CODE = 'CFR') AND (SRAA_SAND.EDW_IUF_TRLR.MetricShipDate - SRAA_SAND.EDW_IUF_TRLR.Ship_Cancel_Date >2) THEN 'Late'
		WHEN not (SRAA_SAND.EDW_IUF_TRLR.SHIP_MODE_CD = 'A' AND SRAA_SAND.EDW_IUF_TRLR.SALES_TERMS_CODE = 'CFR') AND (SRAA_SAND.EDW_IUF_TRLR.MetricShipDate - SRAA_SAND.EDW_IUF_TRLR.CTRT_SHP_DT >2) THEN 'Late'
		Else ' OnTime' END
	where SRAA_SAND.EDW_IUF_TRLR.MetricShipDate is not null;

-- Add Source System		
UPDATE SRAA_SAND.EDW_IUF_TRLR
SET Source_System = CASE
		WHEN trim(SRAA_SAND.EDW_IUF_TRLR.DEST_PO_ID) between '0' and '9999999'THEN 'SCMS'
		WHEN trim(SRAA_SAND.EDW_IUF_TRLR.DEST_PO_ID) not between '0' and '9999999'THEN 'PROMPT'
		END
	where SRAA_SAND.EDW_IUF_TRLR.DEST_PO_ID is not null;
		
--Calculate Days Late 
UPDATE SRAA_SAND.EDW_IUF_TRLR
SET DAYS_LATE= CASE
		WHEN  (SRAA_SAND.EDW_IUF_TRLR.SHIP_MODE_CD = 'A' AND SRAA_SAND.EDW_IUF_TRLR.SALES_TERMS_CODE = 'CFR') Then SRAA_SAND.EDW_IUF_TRLR.MetricShipDate - SRAA_SAND.EDW_IUF_TRLR.Ship_Cancel_Date 
		WHEN not (SRAA_SAND.EDW_IUF_TRLR.SHIP_MODE_CD = 'A' AND SRAA_SAND.EDW_IUF_TRLR.SALES_TERMS_CODE = 'CFR') THEN SRAA_SAND.EDW_IUF_TRLR.MetricShipDate - SRAA_SAND.EDW_IUF_TRLR.CTRT_SHP_DT
		END
	where SRAA_SAND.EDW_IUF_TRLR.MetricShipDate is not null;